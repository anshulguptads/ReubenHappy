...
def binned_rate(df, col, bins=10, positive_label="Yes"):
    if col not in df.columns:
        return pd.DataFrame(columns=[col, "AttritionRate"])
    series = df[col]
    if not np.issubdtype(series.dtype, np.number):
        return pd.DataFrame(columns=[col, "AttritionRate"])
    b = pd.cut(series, bins=bins, include_lowest=True)
    g = df.groupby(b)["Attrition"].apply(lambda s: (s.astype(str) == positive_label).mean()).reset_index(name="AttritionRate")
    g = g.rename(columns={col: f"{col}_bin"})
    return g
...
# Sidebar Filters
cat_cols_all = [c for c in df.columns if df[c].dtype == "object" or str(df[c].dtype).startswith("category")]
sat_cols_all = [c for c in df.columns if ("Satisfaction" in c) and (df[c].dtype != "object")]
...
with tabs[0]:
    ...
    # 3) Boxplot: MonthlyIncome by Attrition (facet by JobRole if present)
...
with tabs[2]:
    ...
    if st.button("Train & Predict on Uploaded Data"):
        if new_file is None:
            ...
            preprocess = ColumnTransformer([
                ("num", SimpleImputer(strategy="median"), [c for c in X_full.columns if X_full[c].dtype != "object"]),
                ("cat", Pipeline([("imputer", SimpleImputer(strategy="most_frequent")),
                                  ("onehot", OneHotEncoder(handle_unknown="ignore", sparse=False))]),
                 [c for c in X_full.columns if X_full[c].dtype == "object"])
            ], verbose_feature_names_out=False)
            ...
            st.download_button("Download predictions CSV",
                               ...
                               file_name="predictions_with_attrition.csv",
                               mime="text/csv")
